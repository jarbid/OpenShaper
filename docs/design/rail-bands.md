...
### Why the default is constrained, not pure

Bands are scored all the way in to the stringer (the alternative — stopping at the rail
zone — hides the deck crown from the score, and the crown is exactly the foam the first
version left behind). Over that span the crown holds far more removable foam than the
rail turn does, so minimising area without a constraint spends bands flattening deck and
gives the rail corner away. Three bands on the golden shortboard:
...
The last column is the one that decides it, and it is the one a whole-section number
hides: `least-foam` wins the headline outright while leaving the rail corner more than
twice as proud as tradition does. `balanced` pins the first band at 45°, where every
reference card already puts it, and lands the corner on the ladder's own figure to six
decimal places — the constraint is binding, so the two are cutting the identical first
facet. It costs half a percent of the foam. The constraint is one-sided: a tight rail
better served by a steeper first band still gets one.
...
rail without a hard constraint. It works, in the sense that it does what it says: equal
angular gaps (`90k/(n+1)` on a circle, the closed form), and **0.33 mm** left at the
corner, finer than anything else on offer.

It shipped in a first cut of this work and was then removed, for two reasons that only
show up in measurement. 0.33 mm is finer than a hand plane holds, so the facet lands
close enough to the finished surface to leave nothing to fair and to depend on the cut
not having wandered — the 1.2 mm the other placements leave is a blending allowance, not
a shortfall. And it pays for that with 60% foam removal against `balanced`'s 95%, because
being scale-blind it spends every band inside the turn and leaves the entire crown to one
gap. An option that is worst on every number the sheet prints, in exchange for a
precision the tool cannot deliver, is one nobody should be choosing — and one that only
gets chosen by mistake.
...
- **Per station** — the real cross-section at **true 1:1**, the leftover foam tinted so
