...
| **JC-4 y-lock** when `adjustCrossectionThickness` off    | **Reproduced (as a pipeline gate)** — modelled not as a per-knot y-mask but as a gate on whether `commit`/`load` run `adjustCrossSectionsToThicknessAndWidth`. The `adjustCrossSectionThickness` editor setting (default on) threads into the store's `adjustThickness` flag; when off, a rocker/deck/outline edit leaves each section's own thickness & width intact. See `docs/specs/divergences.md`. |
